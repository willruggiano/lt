...
pub fn ref_fragment(
    ...
    for (field, role) in fragment.fields.iter().zip(roles.iter()) {
        match role {
            ...
            FieldRole::ScalarColumn { .. } | FieldRole::ForeignKey { .. } | FieldRole::Junction { .. } => {}
        }
    }
...
fn scalar_field_type(rust_base_type: &str, list: bool) -> TokenStream {
    ...
    if list { quote! { Vec<#base> } } else { base }
}
...
mod tests {
    ...
    // Mirrors `lt-types/src/types.rs:21-99`: the `Issue` fragment and its
    // `Parent`/`WorkflowState`/`Team`/`IssueLabelConnection` dependencies.
    const ISSUE_FRAGMENT_SRC: &str = r#"
        #[derive(cynic::QueryFragment, Debug, Clone, PartialEq)]
