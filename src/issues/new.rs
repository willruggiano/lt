...
use crate::linear::mutations::{
    CreateIssueInput, WorkflowState, Team, create_issue, fetch_teams, fetch_workflow_states,
};
...
fn pick_team<'a>(teams: &'a [Team], hint: Option<&str>) -> Result<&'a Team> {
    ...
    loop {
        ...
        println!("Invalid selection. Enter a number (1-{}) or team name.", teams.len());
    }
}
...
fn prompt_description(hint: Option<&str>) -> Result<Option<String>> {
    ...
    let input = read_line("Description (optional, press Enter to skip, type 'e' to open editor): ")?;
    if input.trim().is_empty() {
        return Ok(None);
    }
    if input.trim() == "e" {
...
fn pick_state<'a>(states: &'a [WorkflowState], hint: Option<&str>) -> Result<Option<&'a WorkflowState>> {
    ...
    let default_name = default_state
        .map(|s| s.name.as_str())
        .unwrap_or("(first)");
...
pub fn run(args: NewIssueArgs) -> Result<()> {
    ...
    if let Some(ref sid) = state_id {
        let sname = states.iter().find(|s| &s.id == sid).map(|s| s.name.as_str()).unwrap_or(sid.as_str());
        println!("  State:       {}", sname);
    } else {
    ...
    if let Some(ref aid) = assignee_id {
        let aname = if aid == &viewer.id {
            ...
            members.iter().find(|m| &m.id == aid).map(|m| m.name.clone()).unwrap_or_else(|| aid.clone())
        };
