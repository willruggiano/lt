...
impl App {
    ...
    fn open_new_issue_modal(&mut self) {
        ...
        match crate::linear::mutations::fetch_teams(&token.access_token) {
            Ok(teams) => {
                modal.teams = teams
                    .into_iter()
                    .map(|t| PopupItem { label: t.name.clone(), id: Some(t.id) })
                    .collect();
                // Pre-select team from filter.
                if let Some(ref preset) = preset_team {
                    if let Some(idx) = modal.teams.iter().position(|t| {
                        t.label.to_lowercase().contains(&preset.to_lowercase())
                    }) {
    ...
    fn new_issue_load_states(&mut self) {
        ...
        match crate::linear::mutations::fetch_workflow_states(&token.access_token, &team_id) {
            Ok(states) => {
                modal.states = states
                    .into_iter()
                    .map(|s| PopupItem { label: s.name, id: Some(s.id) })
                    .collect();
    ...
    fn new_issue_load_assignees(&mut self) {
        ...
        let mut items = vec![PopupItem { label: "Unassigned".to_string(), id: None }];
