...
/// App mode -- only popup variants live here.
/// The detail pane (bd-2g8) will add its own variant.
pub enum Mode {
    Normal,
    InputFilter,
...
pub struct App {
    ...
    pub status: Status,
    pub quit: bool,
    // Filter overlay (kept for compatibility; Mode::InputFilter mirrors this)
    pub input_mode: bool,
...
impl App {
    ...
    fn open_state_popup(&mut self) {
        ...
        // Fetch workflow states for the issue's team using the Linear API.
    ...
    fn open_assignee_popup(&mut self) {
        ...
        // Fetch team members via teams query (members sub-field not available in
        // the basic mutations API; use team members query if available, else fall
        // back to an "Unassign" only popup as a stub).
        // For now: provide Unassign + current assignee.
        let mut items: Vec<PopupItem> = vec![
            PopupItem { label: "Unassign".to_string(), id: None },
        ];
        // Fetch team members using the LINEAR_TEAM_MEMBERS_QUERY.
    ...
    fn popup_confirm(&mut self) {
        ...
        // Clone everything the thread needs so we hold no borrows into self.
        ...
        std::thread::spawn(move || {
            ...
            if let Err(_e) = result {
                ...
                // Cannot push error back to TUI without a channel; best-effort revert only.
            }
        });
...
fn optimistic_update_sqlite(
    ...
    // Build a db::Issue mirroring the current issue with the optimistic change.
        ...
        if event::poll(Duration::from_millis(100))? {
            if let Event::Key(key) = event::read()? {
                ...
                match app.mode {
                    ...
                    Mode::Normal => handle_normal_key(&mut app, key.code, key.modifiers),
                }
            }
        }
    }
